...
>
  <div class="flex items-center p-3">
    <button
      type="button"
      class="flex items-center justify-center block w-6 h-6 text-white bg-gray-400 rounded
    ...
    <p class:text-gray-500="{completed}" class:line-through="{completed}" class="px-2">{text}</p>
  </div>
  <div class="flex items-center justify-between flex-1 px-2 bg-white bg-gray-200">
    {#if !completed}
      <PrioritySwitch bind:checked="{priority}" on:change="{changePriorityHandler}" />
